...
class TuiRuntime:
    ...
    def process_once(self) -> RuntimeStatus | None:
        ...
        if processed is None:
            detail = getattr(self._engine, "last_process_error", None) or "Unknown frame error"
            self.last_error = f"Frame loop ended: {detail}"
            self.stop()
            return None
    ...
    def current_model_name(self) -> str:
        if self._engine is not None:
            return self._engine.detector.model_name or self.config.inference.last_model_name
        return self.config.inference.last_model_name
...
def resolve_model_name(runtime: TuiRuntime, status: RuntimeStatus | None) -> str:
    if status is not None and status.model_name:
...
def app_state_from_runtime(runtime: TuiRuntime, logs: tuple[str, ...] = ()) -> AppState:
    ...
    model_name = resolve_model_name(runtime, status)
    camera_index = runtime.config.camera.index if status is None else status.camera_index
    camera_label = format_camera_display(camera_index, runtime.config.camera.source)
    if status is None:
        return AppState(
    ...
    return AppState(
        ...
        camera_label=format_camera_display(status.camera_index, runtime.config.camera.source),
...
def render_header(state: AppState) -> Text:
    ...
    header.append(f"  {state.camera_label}", style="dim")
    header.append("  model ", style="dim")
    header.append(state.model, style="bold bright_white")
    header.append("  fps ")
...
def render_metrics(state: AppState) -> Group:
    ...
    items = [
        _metric_card("Model", state.model, "bold bright_white"),
        _metric_card("Camera", state.camera_label, "bold bright_cyan"),
