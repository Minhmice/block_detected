"""Keyboard and mouse input for the webcam UI."""

import logging

import cv2

from block_detected.runtime.config_schema import AppConfig, InferenceConfig, UiDebugConfig
...
def handle_key(
    ...
    *,
    ...
    """
    Process a key press from waitKeyEx.
    ...
    if key == ui.key_arrow_up:
        if runtime_state.eval_mode:
            ...
            runtime_state.confidence = min(inference.conf_max, runtime_state.confidence + inference.conf_step)
    ...
    if key == ui.key_arrow_down:
        if runtime_state.eval_mode:
            ...
            runtime_state.confidence = max(inference.conf_min, runtime_state.confidence - inference.conf_step)
