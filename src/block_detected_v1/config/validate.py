"""Validation helpers for AppConfig."""
...
def validate_app_config(config: AppConfig) -> list[str]:
    ...
    camera_fields_valid = all(
        (
            require_int("camera.index", config.camera.index),
            require_int("camera.max_index", config.camera.max_index),
            require_int("camera.width", config.camera.width),
            require_int("camera.height", config.camera.height),
        )
    )
    source_valid = config.camera.source in ("auto", "usb", "libcamera", "gstreamer", "rpicam")
    if not source_valid:
    ...
    require_bool("stability.enabled", config.stability.enabled)
    require_number("stability.min_confidence", config.stability.min_confidence)
    require_int("stability.min_box_area_px", config.stability.min_box_area_px)
    require_bool("stability.reject_edge_boxes", config.stability.reject_edge_boxes)
    stability_iou_valid = require_number(
        "stability.duplicate_merge_iou",
        config.stability.duplicate_merge_iou,
    )
    stability_window_valid = require_int("stability.temporal_window", config.stability.temporal_window)
    stability_votes_valid = require_int(
        "stability.required_stable_votes",
        config.stability.required_stable_votes,
    )
    require_str("ui.window_name", config.ui.window_name)
    require_int("ui.button_margin", config.ui.button_margin)
    require_int("ui.button_height", config.ui.button_height)
    require_int("ui.button_pad_x", config.ui.button_pad_x)
    require_int("ui.key_arrow_up", config.ui.key_arrow_up)
    require_int("ui.key_arrow_down", config.ui.key_arrow_down)
    log_level_valid = require_str("ui.log_level", config.ui.log_level)
    ...
    if camera_fields_valid and (config.camera.width < 1 or config.camera.height < 1):
        errors.append("camera width/height must be positive")
    if log_level_valid and config.ui.log_level.upper() not in {
        "DEBUG",
        "INFO",
        "WARNING",
        "ERROR",
